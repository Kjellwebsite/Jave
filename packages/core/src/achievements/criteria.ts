...
import { DOMAIN_EVENT_TYPES, type DomainEventType } from '../events/catalog';
...
/**
 * Domain events that can never drive an achievement. The rule of thumb:
 *  - Discord presence or activity is not capability (joins, leaves, RSVPs, games);
 *  - self-reported or unreviewed actions are farmable (claims, submissions, profile edits);
 *  - negative or disciplinary outcomes are never rewarded (rejections, expiries, moderation);
 *  - events whose outcome lives only in the payload are ambiguous (a verified-rank change may
 *    be a clear, a review may be a rejection), so they cannot be counted as wins;
 *  - operational/system events carry no personal accomplishment;
 *  - achievement events themselves are excluded to keep the engine free of feedback loops.
 * Every other catalog event is eligible, including ones appended to the catalog later.
 */
export const EXCLUDED_ACHIEVEMENT_EVENTS: ReadonlySet<DomainEventType> = new Set<DomainEventType>([
  'member.joined',
  'member.left',
...
/** Event types the achievement engine subscribes to (computed once from the catalog). */
export const ACHIEVEMENT_EVENT_TYPES: readonly DomainEventType[] = DOMAIN_EVENT_TYPES.filter(
  (type) => !EXCLUDED_ACHIEVEMENT_EVENTS.has(type),
);

export function isAchievementEventType(value: string): value is DomainEventType {
...
const eventCountCriteriaSchema = z
  .object({
    ...
    event: z
      ...
      .transform((value) => value as DomainEventType),
